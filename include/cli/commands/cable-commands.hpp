...
/**
 * Cable/Serial connection command handlers.
 */
class CableCommands {
public:
    ...
    static CommandResult cmdCableDisconnect(const std::vector<std::string>& tokens,
        ...
        int devA = findDeviceFunc(tokens[2], devices, -1);
        int devB = findDeviceFunc(tokens[3], devices, -1);
        if (devA < 0 || devB < 0) {
    ...
    static CommandResult cmdCableConnect(const std::vector<std::string>& tokens,
        ...
        int devA = findDeviceFunc(tokens[1], devices, -1);
        int devB = findDeviceFunc(tokens[2], devices, -1);
        if (devA < 0 || devB < 0) {
            result.message = "Invalid device(s)";
            return result;
        }
    ...
    static CommandResult cmdCable(const std::vector<std::string>& tokens,
        ...
        if (tokens[1] == "-d" || tokens[1] == "disconnect") {
            return cmdCableDisconnect(tokens, devices, findDeviceFunc);
        }

        // Connect
        return cmdCableConnect(tokens, devices, findDeviceFunc);
    }
};
