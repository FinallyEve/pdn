 ...
 * Provides commands for testing and debugging device state, allegiance, LEDs, and auto-cycling.
 */
class DebugCommands {
public:
    /**
     * Debug command with multiple subcommands.
     * Supports:
     * - set-npc-state: Force NPC state transitions
     * - set-allegiance: Force player allegiance values
     * - set-led: Manually set LED RGB values
     * - cycle-panels: Toggle auto-cycling through device display panels
     * - cycle-states: Toggle auto-cycling through game states
     * - show-state: Display comprehensive device state summary
     */
    static CommandResult cmdDebug(const std::vector<std::string>& tokens,
                                  ...
                                  bool& g_panelCycling,
                                  int& g_panelCycleInterval,
                                  std::chrono::steady_clock::time_point& g_panelCycleLastSwitch,
                                  bool& g_stateCycling,
                                  int& g_stateCycleDevice,
                                  int& g_stateCycleInterval,
                                  int& g_stateCycleStep,
        ...
        if (subcommand == "set-npc-state") {
            ...
            int deviceIdx = findDeviceFunc(tokens[2], devices, -1);
        ...
        if (subcommand == "show-state") {
            ...
            std::string stateName = getStateNameFunc(stateId, dev.deviceType, dev.gameType);
