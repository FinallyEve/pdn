...
private:
    ...
    static CommandResult cmdHelp(const std::vector<std::string>& /*tokens*/) {
        CommandResult result;
        result.message = "Keys: LEFT/RIGHT=select, UP/DOWN=buttons | Cmds: help, quit, list, select, add, b/l, b2/l2, cable, peer, display, mirror, captions, reboot, role, games, stats, progress, colors, demo, replay, duel, rematch";
        return result;
    }
    
    static CommandResult cmdHelp2(const std::vector<std::string>& /*tokens*/) {
        CommandResult result;
        result.message = "add [hunter|bounty|npc <game>|challenge <game>] - add device | cable <a> <b> - connect | peer <src> <dst> <type> - send packet | reboot [dev] - restart device | games - list games | stats [dev] [summary|detailed|streaks|reset] - FDN game statistics | progress [dev] - Konami grid | colors [dev] - color profiles | demo [game] [easy|hard] - play demo mode | replay [list|<id>|last|clear] - manage replays | duel [history|record|series] - duel tracking | rematch - rematch last opponent";
        return result;
    }
