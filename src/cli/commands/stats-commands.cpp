...
CommandResult StatsCommands::cmdStats(const std::vector<std::string>& tokens,
    ...
    if (subcommand == "reset") {
        return cmdStatsReset(tokens, devices, selectedDevice, findDeviceFunc);
    }
    ...
    int tempDevice = findDeviceFunc(subcommand, devices, -1);
    if (tempDevice >= 0) {
    ...
    // Note: We need to pass getGameDisplayName through the call chain
    // For now, we'll inline it here since we can't pass it through easily
    auto getGameDisplayNameLocal = [](GameType type) -> const char* {
        switch (type) {
    ...
    if (subcommand == "detailed") {
        return cmdStatsDetailed(dev, targetDevice, getGameDisplayNameLocal);
    } else if (subcommand == "streaks") {
        return cmdStatsStreaks(dev, targetDevice, getGameDisplayNameLocal);
    } else {
        // Default: summary
        return cmdStatsSummary(dev, targetDevice, getGameDisplayNameLocal);
    }
}

CommandResult StatsCommands::cmdStatsSummary(DeviceInstance& dev,
                                             int deviceIndex,
    ...
    for (const GameType& game : games) {
        ...
        std::string gameName = getGameDisplayNameFunc(game);
...
CommandResult StatsCommands::cmdStatsDetailed(DeviceInstance& dev,
                                              int deviceIndex,
                                              const char* (*getGameDisplayNameFunc)(GameType)) {
    CommandResult result;
    Player* p = dev.player;
    ...
    for (const GameType& game : games) {
        ...
        output += "┌─ " + std::string(getGameDisplayNameFunc(game)) + " ─────────────\n";
...
CommandResult StatsCommands::cmdStatsStreaks(DeviceInstance& dev,
                                             int deviceIndex,
                                             const char* (*getGameDisplayNameFunc)(GameType)) {
    CommandResult result;
    Player* p = dev.player;
    ...
    for (const GameType& game : games) {
        ...
        snprintf(buf, sizeof(buf), "%-22s Current: %c%-3d | Best Win: %-3d\n",
                 getGameDisplayNameFunc(game), currentChar, currentVal, combined->bestWinStreak);
        output += buf;
    }
...
CommandResult StatsCommands::cmdStatsReset(const std::vector<std::string>& tokens,
    ...
    int targetDevice = selectedDevice;
    if (tokens.size() >= 3) {
        targetDevice = findDeviceFunc(tokens[2], devices, -1);
    }
